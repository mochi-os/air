...
export function normalize_round(
  scene: THREE.Group,
  length = AMRAAM_LENGTH
): THREE.Group {
  ...
  scene.traverse((o) => {
    ...
    for (let i = 0; i < pos.count; i += 3) {
      ...
      const radial =
        axis === 'x'
          ? Math.hypot(v.y - centre.y, v.z - centre.z)
          : axis === 'y'
            ? Math.hypot(v.x - centre.x, v.z - centre.z)
            : Math.hypot(v.x - centre.x, v.y - centre.y)
      if (along < -span * 0.35) {
        low += radial
        lowN++
      } else if (along > span * 0.35) {
        high += radial
        highN++
      }
    }
  ...
  spin.add(scene)
  if (axis === 'x')
    spin.rotation.y = nose_high ? -Math.PI / 2 : Math.PI / 2 // ±x -> +z
  else if (axis === 'y')
    spin.rotation.x = nose_high ? Math.PI / 2 : -Math.PI / 2 // ±y -> +z
...
// from its pylon.
export function amraam_aim(
  root: THREE.Object3D,
  station: number
): THREE.Quaternion {
  const aim = new THREE.Quaternion()
  const source = root.getObjectByName('Missile_' + station)
    ? station
    : station === 3
      ? 2
      : station === 7
  ...
  for (let k = 0; k < total; k += stride) {
    ;(mesh as THREE.Mesh).localToWorld(
