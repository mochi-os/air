  ...
  return (
    <>
      {started && (
        ...
        <LazyBoundary
          onFailure={() => {
            // A chunk that never arrived is gone for the life of the page:
            // lazy() keeps the rejection, so only a reload can recover it. A
            // canvas that threw after loading is a different case, and pressing
            // Fly remounts it, so that one is told what happened and no more.
            setStarted(false)
            leaveFlight()
            toast.error(
              engine === 'failed'
