...
describe('crossHost', () => {
  it('passes an exact match', () => {
    expect(
      crossHost(
        'https://play.example.org:4433/play',
  ...
  it('passes loopback spellings of the same machine', () => {
    ...
    expect(
      crossHost('https://127.0.0.1:4433/play', 'http://localhost:8081')
    ).toBeNull()
    expect(
      crossHost('https://localhost:4433/play', 'http://127.0.0.1:8081')
    ).toBeNull()
    expect(
      crossHost('https://[::1]:4433/play', 'http://localhost:8081')
    ).toBeNull()
    expect(
      crossHost('https://127.0.0.2:4433/play', 'http://127.0.0.1:8081')
    ).toBeNull()
  })
  ...
  it('WARNS when a remote lobby redirects to loopback', () => {
    ...
    expect(
      crossHost('https://127.0.0.1:4433/play', 'https://evil.example.org')
    ).toBe('127.0.0.1')
    expect(
      crossHost('https://localhost:4433/play', 'https://evil.example.org')
    ).toBe('localhost')
  })
  ...
  it('warns on a plain cross-host redirect', () => {
    expect(
      crossHost(
        'https://elsewhere.example.net:4433/play',
        'https://play.example.org'
      )
    ).toBe('elsewhere.example.net')
  })

  it('warns on a private-network redirect from a remote lobby', () => {
    expect(
      crossHost('https://192.168.1.10:4433/play', 'https://play.example.org')
    ).toBe('192.168.1.10')
  })
