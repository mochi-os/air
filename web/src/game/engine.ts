...
import { audio_gesture, audio_enable, audio_frame, audio_gun, audio_hit, audio_explosion, audio_launch, audio_flare, audio_catapult, audio_trap, audio_touchdown, audio_servo, audio_eject, audio_caution, audio_horn, audio_remote, audio_remote_drop, audio_listener } from './audio'
...
function place_on_cat(i=cat_idx){
	...
	const cat=SHIP.shuttles[i], yaw_delta=(SHIP.yaw-90)*D2R, c=Math.cos(yaw_delta), s=Math.sin(yaw_delta);
...
async function init_carrier_model(){
	...
	try{ 
		...
		new GLTFLoader().parse(clean, "", async gltf=>{ try{
			...
			const grp=new THREE.Group(); grp.add(gscene); grp.scale.setScalar(s); grp.rotation.y=SHIP.yaw*D2R; grp.updateMatrixWorld(true);
			...
			const yd=(SHIP.yaw-90)*D2R, dc=Math.cos(yd), ds=Math.sin(yd);                        // rotate the sample spot with the carrier heading
...
function build_carrier_deck_aids(){   // arrestor wires + OLS meatball on the flight deck (called once the carrier GLB has loaded and CARRIER.deckY is known)
	...
	const twfa=SHIP.wires[1], ofa=SHIP.ols.fa, olat=SHIP.ols.lat;   // OLS bracket position on the carrier's port side
...
addEventListener("keydown",e=>{ if(["ArrowUp","ArrowDown","ArrowLeft","ArrowRight"," ","PageUp","PageDown","/"].includes(e.key)) e.preventDefault();
	...
	const k=e.code; if(!keys.has(k)){ // edge-triggered actions
		...
		if(k==="KeyG" && !on_ground()){ ownship.gearTarget = ownship.gearTarget>0.5?0:1; audio_servo(); }   // G: landing gear up/down — only once airborne, never on deck/runway
...
const CARRIER_YD=(SHIP.yaw-90)*D2R, CARRIER_C=Math.cos(CARRIER_YD), CARRIER_S=Math.sin(CARRIER_YD);   // same yaw-delta frame as place_on_cat
...
function fly_player(dt){
	audio_enable(cfg.sound!==false && !pause_toggle);
