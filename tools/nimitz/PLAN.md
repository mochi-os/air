...
- **Stage C — plan-based outline + all equipment (NOT DONE, optional).** Extract `plan.json`, re-source outline/cats/wires/line/elevators/OLS, snap to specs, align touch-up. The remaining wins: exact deck-edge SHAPE (bow point, round-down curves, elevator positions) and equipment placement from an external reference instead of the (squashed) model.
