...
describe('normalize', () => {
  ...
  it('drops unknown fixtures and stores and resizes points', () => {
    const lo = normalize({
      2: { fixture: 'catapult', stores: ['brick'] },
      8: { fixture: 'twin', stores: ['9m', 'brick', '9m'] },
    })
    expect(lo['2']).toEqual({ fixture: '', stores: [] })
    expect(lo['8']).toEqual({ fixture: 'twin', stores: ['9m', ''] })
  })
  it('locks the tips to their rail', () => {
    expect(normalize({ 1: { fixture: '', stores: [] } })['1'].fixture).toBe(
      'rail'
    )
  })
})
...
describe('presets', () => {
  it('fox2 is six rounds: tips plus four wing singles', () => {
    expect(rounds(PRESETS.fox2).map((r) => r.name)).toEqual([
      'tip9',
      'tip1',
      '9m8',
      '9m2',
  ...
  it('fox3 is tip heaters, six AMRAAM singles, and the centreline tank', () => {
    ...
    expect(amraams(PRESETS.fox3)).toEqual([
      '120c4',
      '120c6',
      '120c2',
      '120c8',
...
describe('rounds', () => {
  it('follows the SMS priority order: tips alternating, then outboards, twins outer first', () => {
    ...
    expect(rounds(lo).map((r) => r.name)).toEqual([
      'tip9',
      'tip1',
      '9m8a',
      '9m2a',
...
describe('mask', () => {
  ...
  it('clears bits as rounds fire in order', () => {
    ...
    const has = (m: number, name: string) =>
      Math.floor(m / 2 ** BOOK.index.get(name)!) % 2 === 1
    ...
    for (const name of ['tip1', 'tip9', '9m2', '9m8', '9m3', '9m7'])
      expect(has(spent, name)).toBe(false)
  ...
  it('carries the ten-round fit past bit 31 and sheds fired AMRAAMs in their own order', () => {
    ...
    for (const name of ['120c4', '120c2a', '120c3b', '120c7b'])
      expect(has(full, name)).toBe(true)
    ...
    for (const name of ['120c4', '120c2a', '120c2b', '120c3a', '120c7b'])
      expect(has(dry, name)).toBe(false)
...
describe('weight', () => {
  it('sums hardware and tank capacity', () => {
    ...
    expect(w.hardware).toBe(
      86 * 2 + 179 * 2 + 31 * 2 + 136 * 2 + 120 + 158 + 156 * 6
    )
...
describe('asymmetry', () => {
  ...
  it('one full tank to port is the NATOPS example — over every landing limit', () => {
    ...
    const lopsided = normalize({
      3: { fixture: 'pylon', stores: ['tank'] },
      7: { fixture: 'pylon', stores: [] },
    })
...
describe('outcomes', () => {
  ...
  it('presets are expressible as outcomes', () => {
    for (const preset of Object.values(PRESETS)) {
      for (let station = 1; station <= 9; station++) {
        expect(
          outcomes(station).some(
            (o) => outcome(station, preset[String(station)]) === o.id
          )
        ).toBe(true)
      }
    }
...
describe('entries', () => {
  it('twin points map outer then inner', () => {
    expect(entries(8, { fixture: 'twin', stores: ['9m', '9m'] })).toEqual([
      'twin8',
      '9m8a',
      '9m8b',
    ])
...
describe('amraam (#27)', () => {
  it('cheeks take only the AIM-120C; wing rails and twin points take either family', () => {
    ...
    expect(entries(6, { fixture: 'rail', stores: ['120c'] })).toEqual([
      'rail6',
      '120c6',
    ])
    expect(entries(2, { fixture: 'rail', stores: ['120c'] })).toEqual([
      'rail2',
      '120c2',
    ])
    const lo = normalize({
      4: { fixture: 'rail', stores: ['9m'] },
      2: { fixture: 'twin', stores: ['120c', '120c'] },
    })
  ...
  it('the inboards carry heaters too: singles on the LAU-115C, pairs on its twin rails', () => {
    ...
    const lo = normalize({
      3: { fixture: 'twin', stores: ['9m', '120c'] },
      7: { fixture: 'pylon', stores: ['9m'] },
      5: { fixture: 'pylon', stores: ['9m'] },
    })
    ...
    expect(entries(7, { fixture: 'twin', stores: ['120c', '120c'] })).toEqual([
      'twin7',
      '120c7a',
      '120c7b',
    ])
  ...
  it('the setup offers the pair outcomes and represents hand-built partials', () => {
    ...
    expect(outcome(2, { fixture: 'twin', stores: ['120c', '120c'] })).toBe(
      '120c2'
    )
    ...
    expect(outcome(8, { fixture: 'twin', stores: ['9m', '120c'] })).toBe(
      'mixed'
    )
  ...
  it('the inboard pylons trade fuel for magazine; the centreline never carries one', () => {
    ...
    expect(options(5, 'pylon')).toEqual(['', 'tank'])
    expect(entries(7, { fixture: 'pylon', stores: ['120c'] })).toEqual([
      'pylon7',
      '120c7',
    ])
