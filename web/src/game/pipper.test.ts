...
function fly(
  ...
  let where = { ...muzzle }
  let v = {
    x: bore.x * MUZZLE + own.x,
    y: bore.y * MUZZLE + own.y,
    z: bore.z * MUZZLE + own.z,
  }
  let best = Infinity
  for (let n = 1; n <= 200; n++) {
    ...
    const t = n * step
    const him = {
      x: target.x + drift.x * t,
      y: target.y + drift.y * t,
      z: target.z + drift.z * t,
    }
    ...
    if (square > 1e-9) {
      back = Math.min(
        Math.max(
          (rel.x * close.x + rel.y * close.y + rel.z * close.z) / square,
          0
        ),
        step
      )
    ...
    const at = {
      x: rel.x - close.x * back,
      y: rel.y - close.y * back,
      z: rel.z - close.z * back,
    }
...
describe('aim solves for a hit', () => {
  ...
  const cases: { name: string; range: number; drift: Vector; own: Vector }[] = [
    {
      name: 'point blank, non-manoeuvring',
      range: 150,
      drift: still,
      own: { x: 200, y: 0, z: 0 },
    },
    ...
    {
      name: 'a normal gun shot',
      range: 800,
      drift: { x: 0, y: 0, z: 250 },
      own: { x: 240, y: 0, z: 0 },
    },
    {
      name: 'the range the old clamp still handled',
      range: 1300,
      drift: { x: 0, y: 0, z: 250 },
      own: { x: 240, y: 0, z: 0 },
    },
    {
      name: 'a long shot, inside the reach',
      range: 1900,
      drift: { x: 0, y: 0, z: 200 },
      own: { x: 240, y: 0, z: 0 },
    },
    {
      name: 'well past the old 2 km clamp',
      range: 2600,
      drift: { x: 0, y: 0, z: 150 },
      own: { x: 240, y: 0, z: 0 },
    },
    {
      name: "past the round's reach",
      range: 3600,
      drift: { x: 0, y: 0, z: 150 },
      own: { x: 240, y: 0, z: 0 },
    },
    {
      name: 'a climbing target',
      range: 900,
      drift: { x: 0, y: 80, z: 180 },
      own: { x: 240, y: 0, z: 0 },
    },
    {
      name: 'a target running away',
      range: 1100,
      drift: { x: 260, y: 0, z: 0 },
      own: { x: 240, y: 0, z: 0 },
    },
  ...
  for (const c of cases) {
    it(`hits ${c.name} at ${c.range} m`, () => {
      ...
      if (
        c.range < reach(altitude, size(launch({ x: 1, y: 0, z: 0 }, c.own)))
      ) {
        ...
        // does not carry.
        expect(miss).toBeLessThan(
          c.range -
            reach(altitude, size(launch({ x: 1, y: 0, z: 0 }, c.own))) +
            80
        )
      }
    })
    ...
    it(`draws the pipper on the target once aimed: ${c.name}`, () => {
      ...
      expect(
        Math.hypot(point.x - target.x, point.y - target.y, point.z - target.z)
      ).toBeLessThan(8)
    })
  }
